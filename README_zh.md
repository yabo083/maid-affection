...
使用**锁链**与你的女仆缔结灵魂契约。锁链将成为虚拟生命值池（灵魂护盾）的载体，为你吸收伤害。

**使用方法：**

1. **绑定**：主手持锁链 → **潜行 + 右击**女仆完成绑定
2. **解绑**：对已绑定的女仆执行相同操作即可解除
3. **被动防护**：将绑定后的锁链放在背包中任意位置——当你受伤时，锁链的能量会自动吸收部分伤害
4. **查看红线**：手持绑定的锁链（主手或副手）即可看到连接你和女仆的红色粒子线

**机制详情：**

| 机制 | 详情 |
|---|---|
| **虚拟生命池** | 能量 = (好感度等级 + 1) × 5.0（每位绑定女仆） |
| **伤害分担** | 等级 0：15% → 等级 1：25% → 等级 2：35% → 等级 3：50% |
| **能量回复** | 0.5 能量/秒 |
| **绑定上限** | 每条锁链 1 位女仆（可配置，最高 10） |
| **女仆安全** | 纯虚拟生命值——你的女仆**零实际伤害** |
| **视觉效果** | 持锁链时显示红色粒子线（能量耗尽时变为灰色） |
| **状态指示** | 护盾激活时显示「生命连锁」药水效果图标 |
...
- **数据存储**: 生命连锁数据以 `DataComponentType` 存储在锁链 ItemStack 上（背包中持久化）
- **客户端效果**: FOV 缩放通过 `ComputeFovModifierEvent` + 镜头角度通过 `ViewportEvent.ComputeCameraAngles` 实现
- **伤害系统**: 生命连锁通过拦截 `LivingIncomingDamageEvent` 在伤害生效前减免
